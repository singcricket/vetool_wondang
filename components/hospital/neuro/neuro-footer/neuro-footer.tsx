...
export default function NeuroFooter({ hosId, targetDate }: Props) {
  const { push } = useRouter()
  const path = usePathname()

  // /hospital/:hosId/neuro/:targetDate 이후 세그먼트
  ...
  function handleNav(value: FooterValue) {
    if (value === 'chart') {
      push(`/hospital/${hosId}/neuro/${targetDate}`)
    } else {
      push(`/hospital/${hosId}/neuro/${targetDate}/${value}`)
    }
  }

...
interface NeuroPatientButtonProps {
  item: NeuroSidebarItem
  hosId: string
  targetDate: string
  isActive: boolean
  onClick?: () => void
...
export default function NeuroPatientButton({
  item,
  hosId,
  targetDate,
  isActive,
  onClick,
}: NeuroPatientButtonProps) {
  ...
  return (
    <Button
      ...
      onClick={() => {
        push(`/hospital/${hosId}/neuro/${targetDate}/${item.id}`)
        onClick?.()
      }}
    >
      {/* 환자명 + 차트번호 */}

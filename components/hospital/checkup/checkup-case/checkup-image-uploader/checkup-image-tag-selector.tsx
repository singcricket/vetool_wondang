...
export default function CheckupImageTagSelector({ selectedTags, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {CHECKUP_IMAGE_TAG_GROUPS.map((group) => (
        <div key={group.group}>
          ...
          <div className="flex flex-wrap gap-1">
            {group.tags.map((tag) => {
              ...
              const colorClass = TAG_COLOR_CLASS[getTagColor(tag.id)] ?? TAG_COLOR_CLASS.slate
              return (
                <button
                  key={tag.id}
                  type="button"

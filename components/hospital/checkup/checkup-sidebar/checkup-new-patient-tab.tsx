...
export default function CheckupNewPatientTab({
  hosId,
  targetDate,
  setOpen,
  onRegistered,
  ...
  const handleRegister = async (values: z.infer<typeof registerPatientFormSchema>) => {
    ...
    try {
      const { birth, breed, gender, hos_owner_id, hos_patient_id, name, species, owner_name, microchip_no, memo, weight } = values
      const checkupId = await registerPatientAndCheckupRecord({
        ...
        patient: {
          ...
          microchip_no: microchip_no || undefined,
          memo: memo || undefined,
      ...
      <form onSubmit={form.handleSubmit(handleRegister)} className="grid grid-cols-2 gap-4">
        ...
        <FormField control={form.control} name="species" render={({ field }) => (
          <FormItem>
            ...
            <Select onValueChange={(value) => { field.onChange(value); form.setValue('breed', '') }} defaultValue={field.value}>
              <FormControl>
              ...
              <PopoverTrigger asChild disabled={!watchSpecies}>
                <FormControl>
                  <Button
                    variant="outline"
                    role="combobox"
                    className={cn('relative h-8 w-full justify-start pl-3 font-normal', !field.value && 'text-muted-foreground')}
                  >

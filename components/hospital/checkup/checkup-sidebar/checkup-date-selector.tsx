...
export default function CheckupDateSelector({ hosId, targetDate }: Props) {
  const { push } = useRouter()
  const [open, setOpen] = useState(false)

  const updateDate = (newDate: Date) => {
    push(`/hospital/${hosId}/checkup/${format(newDate, 'yyyy-MM-dd')}`)
  }
    ...
    <div className="flex items-center justify-center gap-1">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button className="h-8 px-2 py-0 font-mono text-base font-semibold" variant="ghost">

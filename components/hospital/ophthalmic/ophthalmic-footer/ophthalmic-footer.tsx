...
export default function OphthalmicFooter({ hosId, targetDate }: Props) {
  const { push } = useRouter()
  const path = usePathname()

  // /hospital/:hosId/ophthalmic/:targetDate 이후 세그먼트
  ...
  function handleNav(value: FooterValue) {
    if (value === 'chart') {
      push(`/hospital/${hosId}/ophthalmic/${targetDate}`)
    } else {
      push(`/hospital/${hosId}/ophthalmic/${targetDate}/${value}`)
    }
  }

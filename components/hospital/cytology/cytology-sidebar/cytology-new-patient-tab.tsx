...
import { registerPatientAndCytologyChart } from '@/lib/services/cytology/register-cytology'
...
export default function CytologyNewPatientTab({ hosId, targetDate, setOpen, onRegistered }: Props) {
  ...
  const form = useForm<z.infer<typeof registerPatientFormSchema>>({
    resolver: zodResolver(
      registerPatientFormSchema.refine(
        async (data) => {
          const isDuplicate = await isHosPatientIdDuplicated(data.hos_patient_id, hosId)
          return !isDuplicate
        },
        { message: '이 환자 번호는 이미 존재합니다', path: ['hos_patient_id'] },
      ),
    ),
    defaultValues: {
      name: '', hos_patient_id: '', species: undefined, breed: undefined,
      gender: undefined, birth: undefined, microchip_no: '', memo: '',
      weight: '', owner_name: '', hos_owner_id: '',
    },
  })
  ...
  useEffect(() => { if (watchBreed) setBreedOpen(false) }, [watchBreed])

  const handleRegister = async (values: z.infer<typeof registerPatientFormSchema>) => {
    setIsSubmitting(true)
    try {
      const { birth, breed, gender, hos_owner_id, hos_patient_id, name, species, owner_name } = values
      const chartId = await registerPatientAndCytologyChart({
        hosId,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleRegister)} className="grid grid-cols-2 gap-4">
        <FormField control={form.control} name="name" render={({ field }) => (
          <FormItem>
            <FormLabel>환자 이름 <RequiredFieldDot /></FormLabel>
            <FormControl><Input {...field} className="h-8" autoComplete="off" /></FormControl>
            <FormMessage className="text-xs" />
          </FormItem>
        )} />

        <FormField control={form.control} name="hos_patient_id" render={({ field }) => (
        ...
        <FormField control={form.control} name="species" render={({ field }) => (
          <FormItem>
            ...
            <Select onValueChange={(v) => { field.onChange(v); form.setValue('breed', '') }} defaultValue={field.value}>
              <FormControl>
          ...
          <FormItem className="flex flex-col justify-end">
            ...
            <Popover open={breedOpen} onOpenChange={setBreedOpen} modal>
              <PopoverTrigger asChild disabled={!watchSpecies}>
                <FormControl>
                  <Button variant="outline" role="combobox" className={cn('relative h-8 w-full justify-start pl-3 font-normal', !field.value && 'text-muted-foreground')}>
                ...
                <Command>
                  ...
                  <CommandList>
                    <ScrollArea className="h-64">
                      <CommandEmpty><p className="py-4 text-center text-xs text-muted-foreground">품종 검색 결과 없음</p></CommandEmpty>
                      <CommandGroup>
                        {BREEDS.map((breed) => (
                          <CommandItem value={breed.eng + '#' + breed.kor} key={breed.id + breed.eng} onSelect={field.onChange} className="text-xs">
                            {breed.kor} ({breed.eng})
                          </CommandItem>
                        ))}
                      </CommandGroup>
        ...
        <FormField control={form.control} name="gender" render={({ field }) => (
          <FormItem>
            ...
            <Select onValueChange={field.onChange} defaultValue={field.value}>
              ...
              <SelectContent>
                {SEX.map((sex) => (
                  <SelectItem key={sex.value} value={sex.value} className="text-xs">{sex.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <FormMessage className="text-xs" />
        ...
        <FormField control={form.control} name="microchip_no" render={({ field }) => (
          <FormItem>
        ...
        <FormField control={form.control} name="hos_owner_id" render={({ field }) => (
          <FormItem>
        ...
        <div className="col-span-2 flex items-center justify-end gap-2">
          <Button type="button" variant="outline" size="sm" disabled={isSubmitting} onClick={() => setOpen(false)}>
            닫기
          </Button>

...
import EchoReportPreview from '@/components/hospital/echocardio/echo-report/echo-report-preview'
...
export const echoChartsColumns = (hosId: string): ColumnDef<EchoChartWithPatient>[] => [
  {
      ...
      return (
        <Dialog>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>심초음파 리포트 미리보기</DialogTitle>
            </DialogHeader>
            <EchoReportPreview 
  ...
  },
  {
    ...
    size: 50,
    cell: ({ row }) => {
      const { id, exam_date } = row.original
      const targetDate = format(new Date(exam_date ?? new Date()), 'yyyy-MM-dd')

      return (
        <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
          <Link
            href={`/hospital/${hosId}/echocardio/${targetDate}/${id}`}
          >

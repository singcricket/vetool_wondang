...
/**
 * 고양이(Feline) 전용 심장초음파 템플릿 생성/수정 다이얼로그
 */
export default function UpsertEchoFelineTemplateDialog({ isEdit, hosId, template, testUIMeta }: Props) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="flex max-h-[90vh] max-w-lg flex-col overflow-hidden p-0 border-orange-200">
        <DialogHeader className="px-4 pt-4 bg-orange-50/50">
          <DialogTitle className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-orange-500" />
            {isEdit ? '고양이 템플릿 수정' : '새 고양이(CAT) 템플릿 만들기'}
          </DialogTitle>
          <VisuallyHidden>
            <DialogDescription />
          </VisuallyHidden>
          ...
          {([
            ...
            <button
              ...
              className={cn(
                ...
                tab === t.key
                  ? 'border-b-2 border-orange-600 font-bold text-orange-600'
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-2 rounded border bg-muted/30 p-3 shadow-inner">
                <input
                  ...
                  placeholder="템플릿 이름 *"
                  className="w-full rounded border bg-white px-2 py-1.5 text-sm font-medium focus:ring-1 focus:ring-orange-500 outline-none"
                />
                <input
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="설명 (선택)"
                  className="rounded border bg-white px-2 py-1.5 text-xs text-muted-foreground focus:ring-1 focus:ring-orange-500 outline-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={isPending || !name.trim()} className="bg-orange-600 hover:bg-orange-700 text-white">
          ...
          {tab === 'order' && (
            <div className="flex flex-col gap-2">
                ...
                >
                  {getSortedFlatItems().map((item: any, index: number) => (
                    <div key={item.keywordID} className="flex items-center gap-2 border-b px-3 py-1.5 last:border-b-0 hover:bg-slate-50 transition-colors">
                      <GripVertical className="drag-handle h-3 w-3 cursor-grab text-muted-foreground hover:text-orange-600 active:cursor-grabbing" />
              ...
              <div className="flex justify-between pt-2">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={isPending} className="bg-orange-600 hover:bg-orange-700">
          ...
          {tab === 'guide' && (
            isEdit ? (
              ...
              <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={isPending || !name.trim()} className="bg-orange-600 hover:bg-orange-700">
...
// ─────────────────────────────────────────────
// 가이드 이미지 관리 탭 (위와 동일하지만 색상 반영을 위해 내포 또는 재사용)
// ─────────────────────────────────────────────
      ...
      {isEditing && (
        <div className="border-t bg-slate-50/50 p-2 flex flex-col gap-2">
          <input
            ...
            placeholder="뷰 이름"
            className="w-full rounded border bg-white px-2 py-1 text-xs focus:ring-1 focus:ring-orange-500 outline-none"
          />
  ...
  return (
    <div className="flex flex-col gap-2 rounded border bg-slate-50 p-3 shadow-inner">
      ...
      <input
        ...
        className="rounded border bg-white px-2 py-1 text-xs focus:ring-1 focus:ring-orange-500 outline-none"
      />

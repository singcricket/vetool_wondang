...
export default function UpsertEchoFelineTemplateDialog({ isEdit, hosId, template, testUIMeta }: Props) {
  ...
  const allowedSectionIds = new Set(LAYOUT_FELINE.sections.map((s) => s.sectionID))

  const sectionOrder: EchoSection[] = (
    (template?.section_order as EchoSection[]) ?? (LAYOUT_FELINE.sections.map((s) => s.sectionID) as EchoSection[])
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="flex h-full w-full max-w-none sm:h-[70vh] sm:max-w-[50vw] flex-col overflow-hidden p-0 border-none sm:border border-orange-200 rounded-none sm:rounded-lg">

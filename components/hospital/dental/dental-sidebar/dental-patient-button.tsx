...
interface DentalPatientButtonProps {
  item: DentalSidebarItem
  hosId: string
  targetDate: string
  isActive: boolean
  onClick?: () => void
...
export default function DentalPatientButton({
  item,
  hosId,
  targetDate,
  isActive,
  onClick,
}: DentalPatientButtonProps) {
  ...
  return (
    <Button
      ...
      onClick={() => {
        push(`/hospital/${hosId}/dental/${targetDate}/${item.id}`)
        onClick?.()
      }}
    >
      {/* 환자명 + 차트번호 */}

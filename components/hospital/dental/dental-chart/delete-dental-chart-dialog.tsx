...
import { deleteDentalChart } from '@/lib/services/dental/delete-dental'
...
export default function DeleteDentalChartDialog({
  chartDetail,
  hosId,
  targetDate,
}: Props) {
  ...
  const handleDelete = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsDeleting(true)
    try {
      await deleteDentalChart(chartDetail.id)
      toast.success('치과 차트가 삭제되었습니다')
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mb-2">
            {patientName}의 {chartDetail.chart_date} 치과 차트를 삭제하시겠습니까?
          </DialogTitle>

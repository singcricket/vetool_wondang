...
import type { DentalPatient } from '@/types/dental/dental-type'
...
export default function DentalPatientEditForm({
  patient,
  patientId,
  setIsDialogOpen,
}: Props) {
  ...
  const form = useForm<z.infer<typeof registerPatientFormSchema>>({
    ...
    defaultValues: {
      ...
      birth: patient.birth ? new Date(patient.birth) : new Date(),
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        {/* 생년월일 */}
        <BirthDatePicker form={form} birth={patient.birth ? new Date(patient.birth) : new Date()} />

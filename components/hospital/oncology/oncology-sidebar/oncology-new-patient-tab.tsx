...
import { registerPatientAndOncologyCase } from '@/lib/services/oncology/register-oncology'
...
export default function OncologyNewPatientTab({
  hosId,
  targetDate,
  setOpen,
  onRegistered,
  ...
  const handleRegister = async (values: z.infer<typeof registerPatientFormSchema>) => {
    ...
    try {
      ...
      const caseId = await registerPatientAndOncologyCase({
        hosId,
        caseDate: targetDate,
      ...
      onRegistered()
      setOpen(false)
      push(`/hospital/${hosId}/oncology/${targetDate}/${caseId}`)
    } catch (e: any) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleRegister)} className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          <FormItem>
            <FormLabel>진단명 <span className="text-xs text-slate-400 font-normal">(선택 — 나중에 수정 가능)</span></FormLabel>
            <Input
              placeholder="예: Canine Lymphoma (B-cell, Multicentric)"
              value={diagnosisName}
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col justify-end">
              <div className="flex items-center gap-2">
                ...
                <HelperTooltip side="right">메인차트에 등록된 환자번호</HelperTooltip>
              </div>
              <FormControl>
        ...
        <div className="col-span-2 flex items-center justify-end gap-2 pt-4">
          ...
          <SubmitButton buttonText="등록" isPending={isSubmitting} className="bg-rose-600 hover:bg-rose-700" />
        </div>
      </form>
    </Form>
  )

...
export default function MsLiveMemoGroup({
  ...
  return (
    <div className="flex w-full flex-col">
      {/* Header */}
      <div className="flex items-center justify-between py-2.5 border-b-2 border-emerald-200">
            ...
            <DialogContent className="max-w-2xl bg-white p-0 overflow-hidden border-none shadow-2xl overflow-y-auto max-h-[90vh]">
              <DialogHeader className="bg-emerald-50 px-6 py-4 border-b border-emerald-100">
                ...
                <DialogDescription className="sr-only">
                  실시간으로 기록된 모든 진행 상황을 타임라인 형태로 확인합니다.
                </DialogDescription>
              </DialogHeader>
      ...
      {!isCollapsed && (
        <>
          {/* Input */}
          <div className="bg-emerald-50/30 p-3 border-b border-emerald-100">
            <div className="relative flex flex-col rounded-xl border-2 border-emerald-200 bg-white shadow-sm focus-within:border-emerald-400 transition-all duration-200">
              <Textarea
                disabled={isUpdating || isUploading}
                placeholder="지금 발생한 일 기록 (Enter: 입력, Shift+Enter: 줄바꿈)"
                id="memo-live"
                value={memoInput}
                ...
                className="w-full min-h-[80px] rounded-t-xl border-0 bg-transparent px-3 py-2.5 text-sm placeholder:text-emerald-300 focus-visible:ring-0 resize-none"
              />
              <div className="flex items-center justify-between border-t border-emerald-50 bg-emerald-50/10 px-2 py-2 rounded-b-xl">
                ...
                <Button
                  size="sm"
                  className="h-8 px-4 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition-colors rounded-lg"

...
interface UltrasoundDateSelectorProps {
  hosId: string
  targetDate: string
}

export default function UltrasoundDateSelector({
  hosId,
  targetDate,
}: UltrasoundDateSelectorProps) {
  const router = useRouter()
  const date = new Date(targetDate)

  const handleDateSelect = (newDate: Date | undefined) => {
    if (newDate) {
      router.push(`/hospital/${hosId}/ultrasound/${format(newDate, 'yyyy-MM-dd')}`)
    }
  }

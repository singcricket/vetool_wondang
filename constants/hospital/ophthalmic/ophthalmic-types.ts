...
export interface OphTreatmentOption {
  id: string
  nameKo: string
  nameEn: string
  clientTerm: string // 보호자 이해를 돕는 순화된 표현
  description?: string
  group?: OphTopicalGroup // topical 카테고리 전용 그룹 분류
  frequency?: string // 투여 횟수 (topical/oral 전용, 예: '1일 4회', '1일 2회')
  info?: string // 제조법 또는 임상 참고 정보 (조제 제제용)
}

export type OphTreatmentCategory = 'topical' | 'oral' | 'procedure' | 'surgery' | 'general'

export interface OphTreatmentData {
  category: OphTreatmentCategory
  selectedIds: string[]
  comment: string
  frequencies?: Record<string, string> // item id → 실제 처방 횟수 (예: '1일 4회')
}
